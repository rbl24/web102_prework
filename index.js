...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        // give img the class game-img
        element.querySelector('img').classList.add('game-img');
...
function filterUnfundedOnly() {
    ...
    const notFullyFunded = GAMES_JSON.filter( (game) => {return game.pledged < game.goal;});

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(notFullyFunded);
    console.log(notFullyFunded.length);
...
function filterFundedOnly() {
    ...
    const fullyFunded = GAMES_JSON.filter( (game) => {return game.pledged >= game.goal;});

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(fullyFunded);
    console.log(fullyFunded.length);
}
...
const description = `A total of \$${totalRaised.toLocaleString()} has been raised for ${totalGames} games. `
    + (numUnfunded > 1 ? `Currently, ${numUnfunded} game remains unfunded. ` : `Currently, ${numUnfunded} games remains unfunded. `) 
